...
namespace MIS.ClientUI.Core
{
	public class DefaultShellResolveService : IShellResolveService
	{
		...
		private MIS404 _404 = new MIS404() { VerticalAlignment = VerticalAlignment.Stretch, HorizontalAlignment = HorizontalAlignment.Stretch };
		/// <summary>
		/// 默认插件外壳解析构造函数
		/// </summary>
			...
			if (newVariable != null)
			{
				...
				if (menuItem != null)
				{
					if (this.ShellResolveFreamEvent != null)
					{
						...
						{
							this.ShellResolveFreamEvent(_404);
						}
					}
				}
			}
